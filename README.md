...
여기서 `admin`은 일반 사용자 역할이 아니라 **개발자인 본인만 사용하는 관리자 계정**을 의미합니다.

Riot API Key는 **최종 사용자에게 노출되지 않으며**, 일반 사용자 기능에서 다뤄지지 않습니다.  
Riot API Key는 백엔드 서버 환경변수로만 관리합니다.  
Riot API Key는 클라이언트 애플리케이션 내부에서 입력, 저장, 처리되지 않습니다.  
...
- 친구들의 경기 데이터를 함께 분석하고 팀 밸런싱에 활용함으로써, 비공개 커스텀 게임 그룹의 실력 향상에도 기여하기

## 운영 및 데이터 처리 관점

- 이 프로젝트는 Riot API Key를 최종 사용자에게 공개하지 않고, 클라이언트 애플리케이션에 영구 저장하지 않는 구조로 설계되어 있습니다.
- Riot API Key는 서버 측 환경변수 `TEAM_BUILDER_RIOT_API_KEY`로 관리하며, 실제 Riot API 요청도 모두 백엔드 서비스에서만 수행합니다.
- 따라서 데스크톱 클라이언트가 Riot API를 직접 호출하는 구조가 아니라, 인증된 백엔드를 통해 필요한 데이터만 조회하는 방식입니다.
- 배포 환경에서는 서버 측 환경 변수와 GCP Secret Manager를 사용하는 운영 방식을 기준으로 보안을 강화하고 있습니다.
- Riot 데이터 적재 기능은 비공개 커스텀 게임 그룹을 위한 소규모 운영 흐름을 전제로 하며, 작은 배치 단위 실행, 이미 조회한 데이터의 재사용, 불필요한 반복 요청 최소화를 기준으로 구성되어 있습니다.
- 최근 경기, 승률, KDA 같은 데이터는 공개 평가나 랭킹 산출용이 아니라, 팀 밸런싱과 자기 플레이 복기를 위한 참고 정보로만 사용합니다.
- 사용자 접근권한 관리, 유저 생성, Riot 데이터 적재, 배치 스케줄러, Firestore 모니터링, Riot API 사용 요청은 모두 `admin` 권한에서만 접근할 수 있습니다.
- 이 `admin` 권한은 개발자인 본인만 사용하는 운영 계정으로 제한합니다.
...
- 같은 데이터는 친구들끼리 자신의 플레이를 돌아보고 개선하는 참고 자료로도 활용합니다.
...
- 플레이어를 신고, 평가, 낙인찍기 위한 별도 채널이나 공개 랭킹 시스템을 제공하지 않습니다.
- 사용 대상 유저에게만 개인적으로 ID/PW를 제공하여 서비스 접근을 제한하고 있습니다.
- 사용 대상 유저들의 대전 데이터만 데이터베이스에 수집하고 있습니다.

## 데이터 수집 및 요청 정책

- Riot API 데이터는 작은 배치 단위로 제한적으로 수집하며, 공식 rate limit을 준수하는 방향으로 운영합니다.
- 이미 조회한 데이터는 Firestore에 저장하고 재사용하여 불필요한 반복 요청을 줄입니다.
- Riot API 및 공식으로 문서화된 Third Party Developer Tools 범위 안에서만 데이터를 조회하는 것을 전제로 합니다.

## 디스클레이머

이 프로젝트는 Riot Games의 공식 후원, 인증 또는 제휴를 받은 서비스가 아닙니다.
Riot Games의 공식 로고를 사용하거나 Riot 승인 프로젝트처럼 보이도록 설계하지 않는 것을 원칙으로 합니다.
...
즉 PyQt 클라이언트가 Riot API를 직접 호출하는 구조가 아니라, 백엔드가 필요한 Riot 데이터를 조회하고 클라이언트는 그 결과를 팀 편성에 활용합니다.
...
- 적재 작업은 소규모 비공개 그룹 운영을 전제로 하며, 작은 배치 실행과 기존 데이터 재사용을 통해 반복 요청을 줄이도록 구성되어 있습니다.
- 이 운영 기능은 개발자인 본인의 `admin` 계정에서만 접근할 수 있습니다.

...
Riot API Key는 소스 코드나 저장소에 보관하지 않고, 발급 후 **로컬 환경에만 보관한 뒤 요청이 필요한 시점에만 직접 입력하는 방식**으로 사용 중입니다.  
추후 Personal Key 운영이 안정화되면 GCP에서 제공하는 **시크릿 환경 변수 기능**을 활용해 배포 환경에서도 키를 안전하게 관리할 예정입니다.
...
- 운영자는 Riot API Key를 필요 시점에만 입력해 최근 경기, 계정 메타데이터, 티어 정보를 Firestore에 적재합니다.
